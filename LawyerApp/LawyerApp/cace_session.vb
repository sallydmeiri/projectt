...
Public Class cace_session
    ...
    Dim path As String
    Private Sub cace_session_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ''تعريف المسار 
        ...
        sql = "SELECT       cace_session.Num, casee.num_in_court, cace_session.num_in_office, cace_session.session_date, cace_session.session_time, cace_session.Notes
FROM   (casee INNER JOIN cace_session ON casee.num_in_office = cace_session.num_in_office)"
        ...
        Txtsession_num.Text = ds.Tables("e").Rows(cr).Item("Num")
        ...
        AxAcroPDF1.src = path & "\Attachmentcace_session\" & Txtsession_num.Text & " " & Txtnum_in_office.Text & ".pdf"
    End Sub
    ...
    Private Sub BtnAdd_Click(sender As Object, e As EventArgs) Handles BtnAdd.Click
        ...
        sql = "insert into cace_session (Num, num_in_office, session_date,session_time, Notes)
...
FROM   (casee INNER JOIN cace_session ON casee.num_in_office = cace_session.num_in_office)"
        ...
        If AxAcroPDF1.src <> Nothing Then
            ''  تخزين ملف البدي اف داخل ملف  اتتشمينت كلبنت باسم رقم الجلسة 

            FileCopy(OpenFileDialog1.FileName, path & "\Attachmentcace_session\" & Txtsession_num.Text & " " & Txtnum_in_office.Text & ".pdf")
...
Label1:
    ...
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        ...
        sql = " update cace_session set  session_date= '" & DTPsession_date.Text & "',session_time='" & Txtsession_time.Text & "',Notes='" & TxtNotes.Text & "' where Num =" & Txtsession_num.Text & " and num_in_office= " & Txtnum_in_office.Text & ""
...
Label1:
    ...
    Private Sub BtnDelet_Click(sender As Object, e As EventArgs) Handles BtnDelet.Click
        If MessageBox.Show("هل انت متاكد انك تريد حذف هذا الجلسة؟", "رسالة تنبيه", MessageBoxButtons.YesNo) = DialogResult.Yes Then
            ...
            path = path.Substring(0, path.LastIndexOf("\bin"))
            cnn.ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & path & "\Lawyer1.mdb"
            sql = "delete from cace_session where Num= " & Txtsession_num.Text & " AND num_in_office= " & Txtnum_in_office.Text & ""
